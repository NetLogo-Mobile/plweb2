...
async function main() {
  ...
  try {
    ...
    await page.goto(`data:text/html,${encodeURIComponent(`<!doctype html><html><body><script type="module">
      import pltxt2htm from 'https://local.test/pltxt2htm.js';
      globalThis.__pltxt2htmFactory = pltxt2htm;
    </script></body></html>`)}`);

    await page.waitForFunction(() => typeof globalThis.__pltxt2htmFactory === 'function');

    await page.evaluate(async () => {
      // @ts-ignore
      const mod = await globalThis.__pltxt2htmFactory({ locateFile: () => 'https://local.test/pltxt2htm.wasm' });
      // @ts-ignore
      globalThis.__pltxt2htmModule = mod;
      // @ts-ignore
      globalThis.__parseCommon = mod.cwrap('common_parser', 'number', ['string']);
    });
    ...
    for (const payload of payloads) {
      const result = await page.evaluate(p => {
        // @ts-ignore
        const mod = globalThis.__pltxt2htmModule;
        // @ts-ignore
        const parse = globalThis.__parseCommon;
        const ptr = parse(p);
