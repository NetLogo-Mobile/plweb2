...
export type AppLanguage = 'Chinese' | 'English' | 'German' | 'Japanese' | 'French'
...
export function normalizeToAppLanguage(rawLanguage?: string | null): AppLanguage | null {
  if (!rawLanguage) {
    return null
  }
...
const defaultLanguage = normalizeToAppLanguage(config.value?.language) ?? detectBrowserLanguage()
