...
export type DisplayType = 'Followers' | 'Following' | 'Mutual' | 'Blocked' | number
...
export interface Summary {
  ...
  ParentID?: string
  /** plweb2 edited */
  Settings?: Record<'Comment-Post' | 'Comment-View', int32> | Record<string, never>
  Supports: int32
  Coauthors: User[]
  ContentID?: ID
  ModelName?: string
...
export type APIData<T> = (T extends object ? T & { $type?: TypeTag } : T) | null
...
export type ResultOf<T> = T extends (q: Param<any>) => Promise<infer R> ? R : never
export type ParamOf<T> = T extends (q: Param<infer P>) => Promise<any> ? P : never
...
export interface Messages {
  ...
  ): Promise<Result<{ Comments: CommentResult[]; Count: int32; Target?: UserInfo }>>
